...
#define DBL(level, x)
    ...
    namespace
    {
        ...
        // Returns true if the specified source arguments has initialized (non
        // default) cut points. Otherwise returns false.
        bool hasCutPoints(const Options::SourceArgs& sargs)
        {
            // Note that the default cutIn is initialized with either plus or
    ...
    void RvSession::applyRvSessionSpecificOptions()
    {
        ...
        if (opts.noSequence)
            m_noSequences = true;
        if (opts.scale != 1.0)
            setScaleOnAll(opts.scale);
        setFPS(opts.defaultfps);
        if (opts.fullscreen)
            fullScreenMode(true);
        setRendererType("Composite");

        if (opts.readerThreads > 0 && opts.readerThreads <= 32)
        {
    ...
    std::string RvSession::lookupMuFlag(std::string key)
    {
    ...
    public:
        ...
        void storeCacheMode(RvSession::CachingMode m) { m_cacheMode = m; }

        RvSession::CachingMode cacheMode() { return m_cacheMode; }
    ...
    {
        if (!m_loadState || !m_loadState->timeToLoad() || m_readingGTO)
            return;
        ...
        if (sources().empty())
        {
            ...
            if (opts.useCache)
                m_loadState->storeCacheMode(RvSession::GreedyCache);
            else if (opts.useLCache)
                m_loadState->storeCacheMode(RvSession::BufferCache);
        }
        ...
        else
        {
            ...
            if (doProcessOpts)
                processOptionsAfterSourcesLoaded(rexc.str() != "");
            ...
            // We delay the after-progressive-loading event after all the
            // sources are loaded
    ...
    void RvSession::readUnorganizedFileList(const StringVector& infiles, bool doProcessOpts, bool merge, const string& tag)
    {
        ...
        if (Options::sharedOptions().delaySessionLoading)
        {
            if (!insources.empty())
            {
                ...
                m_loadState = new LoadState(this, insources, noSeq, doProcessOpts, merge, tag);
        ...
        else
        {
            for (int q = 0; q < insources.size(); q++)
            {
                ...
                const Options::Files& files = sargs.files;
                SequenceNameList inputPatterns = sequencesInFileList(files, noSeq ? FalseSequencePredicate : GlobalExtensionPredicate);
    ...
    void RvSession::processOptionsAfterSourcesLoaded(bool hadLoadError)
    {
        Options& opts = Options::sharedOptions();
    ...
    void RvSession::applySingleSourceArgs(Options::SourceArgs& sargs, SourceIPNode* node)
    {
        ...
        if (sargs.fps != 0)
        {
            ...
            //
            //  If we only have one source loaded, the default views have
            //  adopted the "wrong" fps (ie the one from before we set it with
            //  this option), so adjust them.  BUT, not if the user specified a
            //  session FPS on the command line, since the defaultViews should
            //  not adopt an FPS anyway.
            //
        ...
        if (sargs.selectType != "" && sargs.selectName != "")
        {
            ...
            else if (sargs.selectType == "layer")
            {
                StringVector v(3);
                v[0] = "layer";
            ...
            else if (sargs.selectType == "channel")
            {
                StringVector v(4);
                v[0] = "channel";
    ...
    void RvSession::read(const string& filename, const ReadRequest& request)
    {
    ...
    void RvSession::readSourceHelper(StringVector files, Options::SourceArgs& sargs, bool addContents, bool addToExistingSource)
    {
...

        {
            ...
            if (addToExistingSource)
                addToSourceHelper("", files, sargs);
            else
                for (int i = 0; i < files.size(); i++)
                    addSource(files[i], sargs);
        }

        // We need to update the frame range here because the FastAddSourceGuard
        // set the inputs of the SequenceIPNode after the
        // RvSession::newMediaLoaded is called for the last source.
        updateRange();
    }
        ...
        else
        {
            ...
            else if (isStereoSequence(filename))
            {
                ...
                if (movies.size() == 2)
                {
                    ...
                    if (addToExistingSource)
                        addToSourceHelper("", movies, sargs);
                    else
                                       ...
                                       string mediaRepName, string mediaRepSource)
    {
        ...
        //  Store input parameters
        //
        //  Note that the FileSourceIPNode::storeInputParameters() must be
        //  called before the 'source->propertyChanged(movie)' below otherwise
        //  those input parameters might be missed entirely by
        //  FileSourceIPNode::openMovieTask() (which is executed in a worker
        //  thread) or reset cookies which might lead to file access errors such
        //  as : 'Could not locate "https://.../mp4". Relocate source to fix'
        //
        ...
        // Set the proxy parameters based on the per-source command-line flags
        // if any were specified. These are not essential per say but really
        // improves the user experience in progressive source loading mode. For
        // example, the following SG fields are provided via these per-source
        // command-line flags (+in,+out,+fps) when a user on a ShotGrid site
        // selects 'Play in RV' or 'Play in Screening Room for RV':
    ...
    void RvSession::newMediaLoaded(SourceIPNode* node)
    {
        if (SourceGroupIPNode* group = dynamic_cast<SourceGroupIPNode*>(node->group()))
        {
            ...
            if (!(group->property<StringProperty>("ui.name")))
                group->setUINameFromMedia();
    ...
    // Find and return the source group node corresponding to the requested
    // source media representation if any
    static SourceGroupIPNode* sourceGroupFromSwitchInput(IPNode* switchInput)
    {
    ...
    // Find and return the source node corresponding to the requested source
    ...
    std::string RvSession::sourceMediaRep(const string& srcNodeOrSwitchNodeName)
    {
    ...
    )
    {
        ...
        if (!switchNode)
        {
            ...
            if (srcNode)
            {
                ...
                if (sourceNodes)
                    sourceNodes->push_back(srcNode->name());
            }
            return;
        }
    ...
    // Returns the source media rep switch node associated with the source if
    // any
    std::string RvSession::sourceMediaRepSwitchNode(const string& srcNodeName)
    {
        // Find the Switch node(s) associated with the specified source node if
        // any
    ...
    // Returns the source media rep's switch node's first input associated with
    // the source if any
    std::string RvSession::sourceMediaRepSourceNode(const string& srcNodeName)
    {
        // Find the Switch node(s) associated with the specified source node if
        // any
        std::vector<SwitchIPNode*> switchNodes = findSwitchIPNodes(srcNodeName);
            ...
            if (SourceIPNode* activeSrcNode = sourceFromSwitchInput(switchNode->activeInput()))
            {
                ...
                if (activeSrcNode->mediaRepName() != mediaRepName)
                {
                    ...
                    else
                    {
                        ...
                        {
                            // Otherwise, make sure the active source node's
                            // media is active Note that this will trigger the
                            // multiple source media fallback mechanism if the
                            // current source cannot be activated (ie if it is
                            // not available)
    ...
    void RvSession::write(const string& infilename, const WriteRequest& request)
    {
        ...
        if (ext == "rv")
        {
            ...
            if (writeAsCopy)
                userGenericEvent("before-session-write-copy", filename);
            else
                userGenericEvent("before-session-write", filename);
            ...
            if (writeAsCopy)
                userGenericEvent("after-session-write-copy", filename);
            else
                userGenericEvent("after-session-write", filename);
        }
        else
        {
    ...
    const TwkFB::FrameBuffer* RvSession::currentFB() const
    {
    ...
    SourceIPNode* RvSession::sourceNode(const std::string& sourceName) const
    {
        StringVector tokens;
    ...
    SwitchIPNode* RvSession::switchNode(const std::string& switchNodeName) const
    {
        StringVector tokens;
    ...
    const TwkFB::FrameBuffer* RvSession::currentFB(const std::string& sourceName) const
    {
        if (m_displayImage)
        {
            ...
            //
            //  If user passed in a simple name, and we did not find a match
            //  above, look again and return fb if leading substr matches.
            //
    ...
    bool RvSession::isEmpty() const { return rvgraph().imageSources().empty(); }
    ...
    string RvSession::currentSourceName() const
    {
        string name;
    ...
    void RvSession::readGTOSessionContainer(PropertyContainer* pc)
    {
    ...
    namespace
    {
        ...
        void makeTopLevelSet(PropertyContainer* connections, GTOReader::Containers& containers, set<string>& topLevelSet)
        {
            ...
            for (size_t i = 0; i < containers.size(); i++)
            {
                ...
                {
                    //
                    //  If ui.name *and* has no _ in the name (meaning its one
                    //  of ours?)
                    //

                    if (name.find("_") == string::npos)
                        topLevelSet.insert(name);
                }
            }
        }
...
#define PAD "%06zu"
    ...
    void RvSession::readGTO(const string& infilename, bool merge, Options::SourceArgs& sargs)
    {
        ...
        default:
            ...
            if (m_inputFileVersion > 4)
                ...
                cout << " which corresponds to no RV version";
            cout << endl;
            ...
            for (int i = 0; i < containers.size(); i++)
            {
                ...
                if (m_inputFileVersion == 1 || m_inputFileVersion == 2)
                {
                    if (p == "RVColor" && pver == 1)
                    {
                        ...
                        if (pver != 1)
                        {
                            cout << "WARNING: RVColor node in version 2 "
                                    "session file has unknown version "
                                 << pver << endl;
                        }
                    ...
                    {
                        //
                        // The pixel aspect ratio is now handled by the
                        // RVLensWarp node so we need to move it over.
                        //
                        ...
                        //
                        // Note: using the second slot of the tolinPipeline this
                        // way is fragile and will need updating if the order of
                        ...
                        if (FloatProperty* oldP = pc->property<FloatProperty>("pixel", "aspectRatio"))
                        {
                            ...
                            //
                            // "aspectRatio" is the only property of the old
                            // "pixel" component. Therefore after we copy out
                            // the value we must delete the component.
                            //
                    ...
                    {
                        //
                        //  Used to be part of DisplayGroupIPNode but is now
                        //  part of ViewGroupIPNode
                        //
...

        {
            ...
            for (int i = 0; i < containers.size(); i++)
            {
                ...
                if (def && topLevelSet.count(nodeName) > 0)
                {
                    ...
                    {
                        //
                        //  Its a automatically created node, copy the state
                        //  over
                        //
            ...
            for (int i = 0; i < containers.size(); i++)
            {
                ...
                else if (IPNode* n = graph().findNode(nodeName))
                {
                    ...
                    if (topLevelSet.count(nodeName) > 0)
                    {
                        ...
                        //  automatically.  But this may not be what the user
                        //  saved in the session file, since we allow editing of
                        //  the default views.  So in the usual case, set the
                        //  inputs for the default nodes like other nodes, from
                        //  whatever is specified in the connections list.
                        //
                        //  But two special cases:  If we're merging there's not
                        //  really anything sensible to be done and if there are
                        //  no connections for the default view (IE it's a stub
                        //  session file), so in those cases allow default views
                        //  to build themselves.
                        //
                ...
                //
                //  The node has not been created yet, but it may be later (EG
                //  pipeline group contents), so be sure to add to unused list
                //  so we have a chance to copy state in pass 4.
                //
                {
                    unusedContainers.insert(pc);
                }
            ...
            while (!unusedContainers.empty())
            {
                // Pass 4a
                {
                    ...
                    for (PropertyContainerSet::const_iterator i = unusedContainers.begin(); i != unusedContainers.end(); ++i)
                    {
                        ...
                        {
                            //
                            //  Restore the state from the file. This may
                            //  overwrite any default state created when the
                            ...
                            try
                            {
                                ...
                                {
                                    //
                                    //  Don't recopy sources. They have already
                                    //  been copied above in pass 1 and possibly
                ...
                //
                //  Note this for-loop has to be within the pass4 while() loop
                //  as there are unusedContainers nodes that arent found in
                //  graph until nodes from a previous while-loop iteration have
                //  been readCompleted().
    ...
    void RvSession::readCDL(string name, const string& nodeName, bool activate)
    {
        string filename = pathConform(IPCore::Application::mapFromVar(name));
    ...
    IPNode* RvSession::newNode(const std::string& typeName, const std::string& nodeName)
    {
        ...
        if (node)
            userGenericEvent("new-node", node->name());

        return node;
    }

    void RvSession::setRendererType(const std::string& type)
    {
    ...
    void RvSession::deleteNode(IPNode* node)
    {
        ...
        if (isSource)
            userGenericEvent("before-source-delete", name);

        if (node == m_conductorSource)
            m_conductorSource = nullptr;
        if (node == m_sequenceIPNode)
    ...
    //
    void RvSession::setSequenceEvents()
    {
    ...
    //
    void RvSession::unsetSequenceEvents()
    {
        if (!evUseSequenceEventsTracking.getValue())
            return;
    ...
    //
    void RvSession::setSequenceIPNode(IPCore::SequenceIPNode* newSequenceIPNode)
    {
    ...
    //
    void RvSession::onSequenceChanging()
    {
        auto root = rootNode();
    ...
    // sequence IP node.
    //
    // It loads the new state of the source where the playhead was  before the
    // layout and replaces the playhead to the same absolute position if the
    // source is discovered or the same relative position if not.
    //
    // It changes the playhead position to avoid that it dances in the timeline.
    void RvSession::onSequenceChanged()
    {
        if (m_currentSourceIndex < 0)
            return;
        auto root = rootNode();
        ...
        {
            // we want that the playhead stays at the same absolute position in
            // the source
