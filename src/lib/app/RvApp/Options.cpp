...
namespace Rv
{
    ...
    void debugSwitches(const std::string& name)
    {
        ...
        else if (name == "audio")
        {
            ...
            TwkMovie::ResamplingMovie::setDebug(true);
        }
        ...
        else if (name == "dumpaudio")
        {
            ...
            TwkMovie::ResamplingMovie::setDumpAudio(true);
        }
        ...
        else if (name == "shadercode")
        {
            ...
            TwkApp::setDebugMUC(true);
        else if (name == "compile")
            TwkApp::setCompileOnDemand(true);
        else if (name == "dtree")
        ...
        else
        {
            ...
            cout << "INFO: debugging message \"" << name << "\"" << endl;
        }
    }
    ...
    int parseDebugKeyWords(int argc, char* argv[])
    {
        Options& opts = Options::sharedOptions();
    ...
    int parseMuFlags(int argc, char* argv[])
    {
        Options& opts = Options::sharedOptions();
        ...
        for (int i = 0; i < argc; i++)
        {
            ...
            else if (2 == parts.size()) //  name=value flag
            {
                opts.muFlags[parts[0]] = parts[1];
            }
            else
                cerr << "ERROR: mal-formed flag: " << argv[i] << endl;
        }

        return 0;
    }

    int parseSendEvents(int argc, char* argv[])
    {
        Options& opts = Options::sharedOptions();
        ...
        for (int i = 0; i < argc; i += 2)
        {
            ...
            // cerr << "parseSendEvents name=" << e.name << " content=" <<
            // e.content << endl;
        }
    ...
    Options::Options()
    {
        ...
        exrReadWindow = 3;
        exrPlanar3Chan = 0; /* Packed buffers are more optimal. Based on
                               performance tuning */
        exrStripAlpha = 0;
        exrConvertYRYBY = 0;
        jpegRGBA = 0;
            ...
            else if (inSource && arg.size() && arg[0] == '+' && (i + 1 < inputFiles.size()))
            {
                ...
                else if (i + 1 < inputFiles.size())
                {
                    ...
                    if (arg == "+fps")
                        ...
                        a.volume = atof(v);
                    else if (arg == "+audioOffset" || arg == "+ao")
                        a.audioOffset = atof(v);
                    else if (arg == "+rangeOffset" || arg == "+ro")
                        a.rangeOffset = atoi(v);
                    else if (arg == "+rangeStart" || arg == "+rs")
                        a.rangeStart = atoi(v);
                    else if (arg == "+pixelAspect" || arg == "+pa")
                        a.pixelAspect = atof(v);
                    else if (arg == "+stereoOffset" || arg == "+so")
                        a.stereoRelativeOffset = atof(v);
                    else if (arg == "+rightEyeStereoOffset" || arg == "+rso")
                        a.stereoRightOffset = atof(v);
                    else if (arg == "+fcdl")
                        a.fcdl = v;
                    else if (arg == "+lcdl")
                        a.lcdl = v;
                    ...
                    {
                        a.selectType = v;
                        a.selectName = inputFiles[i + 1].c_str();
                        i++;
                    }
                    else if (arg == "+mrn" || arg == "+mediaRepName")
                        a.mediaRepName = v;
                    else if (arg == "+mrs" || arg == "+mediaRepSource")
                        a.mediaRepSource = v;
                    else if (arg == "+in")
                        a.cutIn = atoi(v);
                    else if (arg == "+out")
                        a.cutOut = atoi(v);
                    else if (arg == "+inparams")
                        i += Rv::collectParams(a.inparams, inputFiles, i);
    ...
    void Options::integrateFrameRangeStrings(vector<string>& inputFiles)
    {
    ...
    void Options::exportIOEnvVars() const
    {
        //
    ...
    bool Options::initializeAfterParsing(Options* prefs)
    {
        //
        ...
        if (bgpattern)
        {
            ...
            if (!strcmp(bgpattern, "black"))
                p = ImageRenderer::Solid0;
            else if (!strcmp(bgpattern, "grey18"))
                p = ImageRenderer::Solid18;
            else if (!strcmp(bgpattern, "grey50"))
                p = ImageRenderer::Solid50;
            else if (!strcmp(bgpattern, "white"))
                p = ImageRenderer::Solid100;
            else if (!strcmp(bgpattern, "checker"))
                p = ImageRenderer::Checker;
            else if (!strcmp(bgpattern, "crosshatch"))
                p = ImageRenderer::CrossHatch;
        ...
        if (eval)
            RvSession::setInitEval(eval);
        if (pyeval)
            RvSession::setPyInitEval(pyeval);
        if (maxbits != 32)
            FormatIPNode::defaultBitDepth = maxbits;
        if (nofloat)
            FormatIPNode::defaultAllowFP = false;
        ...
        {
        case 32:
            audioParams.format = TwkAudio::Float32Format;
            break;
        case -32:
            audioParams.format = TwkAudio::Int32Format;
            break;
        case 24:
            audioParams.format = TwkAudio::Int24Format;
            break;
        default:
        case 16:
            audioParams.format = TwkAudio::Int16Format;
            break;
        case 8:
            audioParams.format = TwkAudio::Int8Format;
            break;
        }
        ...
        {
            cerr << "ERROR: you can only have one of -wipe, -over, -diff, "
                    "-replace, -layer, -topmost or -tile"
                 << endl;
            exit(-1);
        }
        ...
        if (sessionType)
        {
            if (!strcmp(sessionType, "sequence"))
                view = (char*)"defaultSequence";
            else if (!strcmp(sessionType, "stack"))
                view = (char*)"defaultStack";
        }
    ...
    void Options::manglePerSourceArgs(char** argv, int argc)
    {
        bool in = false;
        ...
        for (int i = 0; i < argc; ++i)
        {
            ...
            if (!strcmp(s, "["))
                ...
                in = false;
            else if (in && s[0] == '-' && atof(s) >= 0)
                s[0] = '+';
        }
    }

    void Options::output(ostream& out)
    {
        static const char* intfields[] = {"acachesize",
                                          "aframesize",
                                          "allowYUV",
                                          "apple",
                                          "audioNice",
        ...
        static const char* floatfields[] = {"audioMaxCache",
                                            "audioMinCache",
                                            "audioOffset",
                                            "audioRate",
                                            "brightness",
