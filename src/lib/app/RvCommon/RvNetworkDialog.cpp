...
#define DBL(level, x)     \
    if (level & DB_LEVEL) \
    cerr << "NetworkDialog " GPID() << ": " << x << endl
#else
#define DB(x)
#define DBL(level, x)
#endif
...
namespace Rv
{
    ...
    void RvNetworkDialog::loadSettings()
    {
        ...
        for (int i = 0; i < 4; i++)
            m_ui.contactTreeView->resizeColumnToContents(i);
    ...
    int RvNetworkDialog::findContact(const QString& cname, const QString& chost)
    {
                                     ...
                                     const QString& capp, bool checkForExisting)
    {
        ...
        if (tperms == CurrentConnect)
            tperms = m_ui.permissionCombo->currentIndex();

        QString permText = "";
        if (tperms == AskConnect)
            permText = "Ask";
        else if (tperms == AllowConnect)
            permText = "Allow";
        else if (tperms == DenyConnect)
            permText = "Deny";
        else
        {
        ...
        //
        //  If this is a local connection, hide this row in the contact list,
        //  since we always allow them anyway, and display drivers, etc can
        //  clutter the contact list with lots of connections.
        //
    ...
    void RvNetworkDialog::saveSettings()
    {
        RV_QSETTINGS;
        settings.beginGroup("Network");
        ...
        for (int i = 0; i < n; i++)
        {
            ...
            // for local connections, which we don't need to store (since we
            // always allow them).
            //
    ...
    void RvNetworkDialog::shutdownServer()
    {
    ...
    void RvNetworkDialog::savePortNumber()
    {
        DB("savePortNumber");
        ...
        //
        //  Write port number to file named:
        //  <tmp>/tweak_rv_proc/<processID>_<tag>
        //

        QString tagString("_");
        const Rv::Options& opts = Rv::Options::sharedOptions();
        if (opts.networkTag)
            tagString += opts.networkTag;
    ...
    {
        if (!m_portFile.isEmpty())
            QFile::remove(m_portFile);
    }

    void RvNetworkDialog::closeEvent(QCloseEvent* event)
    {
        saveSettings();
        QWidget::closeEvent(event);
    }

    void RvNetworkDialog::toggleServer()
    {
    ...
    void RvNetworkDialog::newRemoteContact(const QString& name)
    {
    ...
    void RvNetworkDialog::startConnect() { startConnectDialog(""); }

    void RvNetworkDialog::connectByName(string host, int port)
    {
    ...
    void RvNetworkDialog::startConnectDialog(const QString& name)
    {
        //
    ...
    void RvNetworkDialog::popup(const QPoint& pos)
    {
    ...
    void RvNetworkDialog::popupDisconnect(bool)
    {
        set<int> rows = selectedContacts();
    ...
    void RvNetworkDialog::popupConnect(bool)
    {
        set<int> rows = selectedContacts();
    ...
    void RvNetworkDialog::popupDelete(bool)
    {
        //
    ...
    void RvNetworkDialog::doubleClickContact(const QModelIndex& index)
    {
        //
    ...
    vector<string> RvNetworkDialog::connections() const
    {
        vector<string> c;
    ...
    vector<string> RvNetworkDialog::sessions()
    {
        vector<string> c;
    ...
    vector<string> RvNetworkDialog::applications() const
    {
        vector<string> c;
    ...
    int RvNetworkDialog::myPort() { return m_ui.portLineEdit->text().toInt(); }

    void RvNetworkDialog::sessionDeleted(const char* sessionName)
    {
    ...
    void RvNetworkDialog::spoofConnectionStream(const QString& fileName, float timeScale, bool verbose)
    {
        ...
        if (!stream->spoofStream(UTF8::qconvert(fileName), verbose))
            return;
