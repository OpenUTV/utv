...
namespace Rv
{
    ...
    bool PackageManager::allowLoading(Package& package, bool allow, int depth)
    {
        ...
        //
        //  Don't read/write settings unless this is the UTV.app (as opposed to
        //  rvpkg).
        //
        ...
        if (findPackageIndexByZip(infileNonCanonical) == -1)
        {
            ...
            if (file)
            {
                ...
                if (!buffer.empty())
                {
                    ...
                    static const bool isUTV = std::string_view(INTERNAL_APPLICATION_NAME) == "UTV";

                    package.compatible = versionCompatible(isUTV ? package.openrvversion : package.rvversion, TWK_DEPLOY_MAJOR_VERSION(),
