...
namespace TwkApp
{
    ...
    void QTBundle::initializeAfterQApplication()
    {
        if (!m_initialized)
            init();
    }
    ...
    void QTBundle::init()
    {
        ...
        m_etc = m_root;
        m_ui = m_root;
        if (!m_scripts.cd("scripts"))
            cerr << "WARNING: missing scripts dir in bundle" << endl;
        if (!m_plugins.cd("plugins"))
            cerr << "WARNING: missing plugin dir in bundle" << endl;

        // Note: we do not issue a warning here for a potentially missing 'etc'
        // dir since it is optional and is not populated on all platforms like
        ...
        for (size_t i = 0; i < m_supportPath.size(); i++)
        {
            ...
            //  cerr << "supp path " << i << ": '" <<
            //  dir.absolutePath().toUtf8().constData() << "'" << endl;
...
//
    ...
    Bundle::Path QTBundle::resource(const FileName& name, const FileName& type)
    {
        if (!m_initialized)
            init();
    ...
    Bundle::Path QTBundle::executableFile(const FileName& name)
    {
        if (!m_initialized)
            init();
        string n = name;
    ...
    Bundle::Path QTBundle::application(const FileName& name)
    {
        if (!m_initialized)
            init();
        //
    ...
    Bundle::PathVector QTBundle::supportPath()
    {
        PathVector paths;
    ...
    Bundle::PathVector QTBundle::pluginPath(const DirName& pluginType)
    {
        if (!m_initialized)
            init();
    ...
    Bundle::PathVector QTBundle::scriptPath(const DirName& scriptType)
    {
        if (!m_initialized)
            init();
        PathVector paths;
    ...
    QTBundle::Path QTBundle::appPluginPath(const DirName& pluginType)
    {
        if (!m_initialized)
            init();
    ...
    QTBundle::Path QTBundle::appScriptPath(const DirName& pluginType)
    {
        if (!m_initialized)
            init();
    ...
    QTBundle::Path QTBundle::userCacheDir()
    {
        QString s = m_homeSupport.absoluteFilePath("Caches");
        return s.toUtf8().constData();
    }

    void QTBundle::setEnvVar(const EnvVar& var, const Path& value, bool force)
    {
    ...
    void QTBundle::addPathToEnvVar(const EnvVar& var, const PathVector& value)
    {
        ostringstream str;
        const char* envvar = getenv(var.c_str());
        if (envvar)
    ...
    Bundle::Path QTBundle::userPluginPath(const DirName& pluginType)
    {
