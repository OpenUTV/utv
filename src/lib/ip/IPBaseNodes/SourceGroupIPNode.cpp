...
namespace IPCore
{
    ...
    SourceGroupIPNode::SourceGroupIPNode(const std::string& name, const NodeDefinition* def, IPGraph* graph, GroupIPNode* group)
    ...
    SourceGroupIPNode::SourceGroupIPNode(const std::string& name, const NodeDefinition* def, SourceIPNode* sourceNode, IPGraph* graph,
                                         GroupIPNode* group)
        : GroupIPNode(name, def, graph, group)
    ...
    void SourceGroupIPNode::setUINameFromMedia(int index)
    {
        ...
        string uiname = (m_sourceNode && m_sourceNode->numMedia() > 0)
                            ? boost::filesystem::path(m_sourceNode->mediaName(index)).filename().string()
                            : "";
        smatch movMatch;
    ...
    void SourceGroupIPNode::init(const std::string& name, const NodeDefinition* def, SourceIPNode* sourceNode, IPGraph* graph,
                                 GroupIPNode* group)
    {
        HOP_PROF_DYN_NAME(std::string(std::string("SourceGroupIPNode::init : ") + name).c_str());
        ...
        string cacheLUTType = def->stringValue("defaults.preCacheLUTType", "CacheLUT");
        string formatType = def->stringValue("defaults.formatType", "Format");
        string channelMapType = def->stringValue("defaults.channelMapType", "ChannelMap");
        string cacheType = def->stringValue("defaults.cacheType", "Cache");
        string paintType = def->stringValue("defaults.paintType", "Paint");
        string overlayType = def->stringValue("defaults.overlayType", "Overlay");
        string colorPipelineType = def->stringValue("defaults.colorPipelineType", "PipelineGroup");
        string linPipelineType = def->stringValue("defaults.linearizePipelineType", "PipelineGroup");
        string lookPipelineType = def->stringValue("defaults.lookPipelineType", "PipelineGroup");
        string stereoType = def->stringValue("defaults.stereoType", "SourceStereo");
        string rotateType = def->stringValue("defaults.rotateType", "RotateCanvas");
        string transformType = def->stringValue("defaults.transformType", "Transform2D");
        string cropType = def->stringValue("defaults.cropType", "Crop");
        string sourceType = def->stringValue("defaults.sourceType", "FileSource");

        CacheLUTIPNode* cacheLUT = newMemberNodeOfType<CacheLUTIPNode>(cacheLUTType, "cacheLUT");
        IPNode* format = 0;
        ChannelMapIPNode* chmap = newMemberNodeOfType<ChannelMapIPNode>(channelMapType, "channelMap");
        CacheIPNode* cache = newMemberNodeOfType<CacheIPNode>(cacheType, "cache");
        PipelineGroupIPNode* tolinGroup = newMemberNodeOfType<PipelineGroupIPNode>(linPipelineType, "tolinPipeline");
        PaintIPNode* paint = newMemberNodeOfType<PaintIPNode>(paintType, "paint");
        OverlayIPNode* overlay = newMemberNodeOfType<OverlayIPNode>(overlayType, "overlay");
        PipelineGroupIPNode* colorGroup = newMemberNodeOfType<PipelineGroupIPNode>(colorPipelineType, "colorPipeline");
        PipelineGroupIPNode* lookGroup = newMemberNodeOfType<PipelineGroupIPNode>(lookPipelineType, "lookPipeline");
        SourceStereoIPNode* stereo = newMemberNodeOfType<SourceStereoIPNode>(stereoType, "sourceStereo");
        RotateCanvasIPNode* rotate = 0;
        Transform2DIPNode* tform = newMemberNodeOfType<Transform2DIPNode>(transformType, "transform2D");
        CropIPNode* crop = 0;
        if (!sourceNode)
            sourceNode = newMemberNodeOfType<SourceIPNode>(sourceType, sourceType == "FileSource" ? "file" : "image");
        ...
        if (rotateType != "")
            rotate = newMemberNodeOfType<RotateCanvasIPNode>(rotateType, "rotate");
    ...
    void SourceGroupIPNode::readCompleted(const std::string& type, unsigned int version) {}
