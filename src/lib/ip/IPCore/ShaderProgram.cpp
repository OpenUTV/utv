...
namespace IPCore::Shader
{
    ...
    bool Program::compile()
    {
        ...
        if (m_needOutputST)
        {
            code << varying << " vec2 _fragCoord;\n";
        }
        if (m_needFragmentPosition)
        {
        ...
        if (m_needFragmentPosition)
        {
            vertexCode << varying_out << " vec3 _fragPosition;\n";
        }
        ...
        if (m_needFragmentPosition)
        {
            // Even though the z component is always 0, we use a vec3 to make it
            // clear it's a position in our "3D" model coordonates system
            vertexCode << "    _fragPosition = vec3( in_Position, 0.0 );\n";
        }
    ...
    const Program* ProgramCache::select(const Expression* A)
    {
        ...
        else
        {
            ...
            if (!p)
                TWK_THROW_EXC_STREAM("ERROR: failed to select the GL program");
            return p;
        }
    }
