...
namespace TwkUtil
{
    ...
    FILE* fopen(const char* c, const char* mode) { return ::fopen(c, mode); }

    int open(const char* c, int oflag) { return ::open(c, oflag); }

    int access(const char* path, int mode) { return ::access(path, mode); }
#endif
...
// mR - 10/28/07
    ...
    void varExp(string& str)
    {
        ...
        for (; posA != string::npos; posA--)
        {
            ...
            if (posA > 0 && !slashmode && str[posA - 1] == '\\')
                slashmode = true;
            if (slashmode)
            {
                slashmode = false;
    ...
    bool filesInDirectory(const char* directory, FileNameList& fileList, bool showDirs)
    {
        ...
        {
            if (!showDirs && d->d_type == DT_DIR)
                continue; // Do not return directories
    ...
    void makeSplitNumberedStrings(const FileNameList& files, SplitNumberedStrings& parts)
    {
        ...
        for (size_t i = 0; i < files.size(); i++)
        {
            ...
            while (Match m = Match(maybePaddedNumberRE, file))
            {
                ...
                if (pos != 0)
                    p0 = file.substr(0, pos);
                file = file.substr(pos + num.size());

                if (p0 != "")
                    element.parts.push_back(NumString(p0));
    ...
    void lexiNumericFileSort(FileNameList& files)
    {
...
#ifdef PLATFORM_WINDOWS
    int runAsAdministrator(std::string command, std::string directory)
    {
        HWND hwnd = NULL;
