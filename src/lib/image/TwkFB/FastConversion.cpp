...
//------------------------------------------------------------------------------
// Converts semi-planar format P210 to planar YUV 422 16-bits.
//
...
//
//  AV_PIX_FMT_P210LE && dstFormat -> AV_PIX_FMT_YUV422P16LE
//  AV_PIX_FMT_P416LE && dstFormat -> AV_PIX_FMT_YUV444P16LE
//
                                   ...
                                   size_t outStrideCb, size_t outStrideCr)
{
    ...
    const uint16_t* FASTMEMCPYRESTRICT startInCbCr = inCbCr;
    uint16_t* FASTMEMCPYRESTRICT startOutY = outY;
    uint16_t* FASTMEMCPYRESTRICT startOutCb = outCb;
    uint16_t* FASTMEMCPYRESTRICT startOutCr = outCr;
    ...
    for (size_t i = 0; i < height; ++i)
    {
        ...
        inCbCr = startInCbCr + i * inStrideCbCr / 2;
...
//------------------------------------------------------------------------------
// Converts semi-planar format P416 to planar YUV 444 16-bits.
//
// Context: To optimize the following FFmpeg formats conversion which is
// required for Apple ProRes decoding on macOS via VideoToolbox since the
// sws_scale() performance for the same conversions was way too slow.
