...
namespace
{
    ...
    float truncateFloat(float f) { return floor(f * 100) / 100; }

    void setFBAttributes(TwkFB::FrameBuffer* fb, LibRaw* ip, string primaries)
    {
...
namespace TwkFB
{
        ...
        : FrameBufferIO("IOraw", "n")
    {
        ...
        addType("nef", "Nikon Electronic Format", capabilities,
                codecs); // tested
    ...
    void IOraw::readImage(FrameBuffer& fb, const std::string& filename, const ReadRequest& request) const
    {
        ...
        if (scale_factor == -1.0)
        {
            ...
            //
            // Lastly make sure the derived scale factor is reasonable. "Should
            // be between 1.0 and 2.8. If its outside this range then the number
        ...
        //  using non-planar FB, avoiding a copy.  But we know that some cards
        //  are slow displaying these textures, so stick to planar FBs in all
        //  cases for now.
        //
        ...
        /*
           ...
           << " flip: " << iProcessor.imgdata.sizes.flip << " flash_used: " <<
           iProcessor.imgdata.color.flash_used << " model2: " <<
           iProcessor.imgdata.color.model2 << " profile: " <<
           iProcessor.imgdata.color.profile << " iso_speed: " <<
           iProcessor.imgdata.other.iso_speed << " shutter: " <<
           iProcessor.imgdata.other.shutter << " aperture: " <<
           iProcessor.imgdata.other.aperture << " focal_len: " <<
           iProcessor.imgdata.other.focal_len << " timestamp: " <<
           iProcessor.imgdata.other.timestamp << " shot_order: " <<
           iProcessor.imgdata.other.shot_order << " gpsdata: " <<
           iProcessor.imgdata.other.gpsdata << " desc: " <<
           iProcessor.imgdata.other.desc << " artist: " <<
           iProcessor.imgdata.other.artist << endl;
        */
        if (!planar)
        {
    ...
    void IOraw::getImageInfo(const std::string& filename, FBInfo& info) const
    {
        //
