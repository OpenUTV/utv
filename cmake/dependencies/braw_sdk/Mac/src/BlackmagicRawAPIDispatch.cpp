...
{
public:
    inline BmdMutex() { pthread_mutex_init(&m_mutex, NULL); }
...
{
public:
    inline explicit BmdScopedLock(BmdMutex& mutex)
        : m_mutex(mutex)
    {
        m_mutex.lock();
    }
...
static void TryLoadBlackmagicRawAPI(CFURLRef bundleURL)
{
    ...
    if (gBundleRef != NULL)
    {
        gCreateBlackmagicRawFactoryInstance =
            (CreateRawFactoryFunc)CFBundleGetFunctionPointerForName(gBundleRef, CFSTR("CreateBlackmagicRawFactoryInstance"));
...
IBlackmagicRawFactory* CreateBlackmagicRawFactoryInstanceFromBundle(void)
{
    ...
    if (gCreateBlackmagicRawFactoryInstance == NULL)
    {
        ...
        if (mainBundle != NULL)
        {
            ...
            {
                CFURLRef bundleURL = CFURLCreateCopyAppendingPathComponent(kCFAllocatorDefault, frameworksBundleURL,
                                                                           CFSTR(kBlackmagicRawAPI_FrameworkName), false);
...
IBlackmagicRawFactory* CreateBlackmagicRawFactoryInstanceFromPath(CFStringRef loadPath)
{
    ...
    if ((loadPath != NULL) && (gCreateBlackmagicRawFactoryInstance == NULL))
    {
        ...
        {
            CFURLRef bundleURL =
                CFURLCreateCopyAppendingPathComponent(kCFAllocatorDefault, pathURL, CFSTR(kBlackmagicRawAPI_FrameworkName), false);
            if (bundleURL != NULL)
            {
                TryLoadBlackmagicRawAPI(bundleURL);
                CFRelease(bundleURL);
...
IBlackmagicRawFactory* CreateBlackmagicRawFactoryInstanceFromIdentifier(CFStringRef identifier)
{
    BmdScopedLock lock(gBlackmagicRawMutex);

    if (gCreateBlackmagicRawFactoryInstance == NULL)
    {
        CFBundleRef mainBundle = CFBundleGetBundleWithIdentifier(identifier);
        if (mainBundle != NULL)
        {
            CFRetain(mainBundle);
            CFURLRef frameworksBundleURL = CFBundleCopyPrivateFrameworksURL(mainBundle);
